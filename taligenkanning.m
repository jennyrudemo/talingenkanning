...
for i=1:antalOrd
    ...
    for j=1:refMangd
        ...
        refkoeffs = mfcc(referensord,fs);
        refValdaKoeffs = transpose(refkoeffs(:,1:koeff)); %Önskat antal koefficienter väljs ut
        
        %Beräkna avstånde mellan testordets och referensordets
        %MFCC-koeficcienter
        distans=dtw(testValdaKoeffs, refValdaKoeffs);
...
%Hitta motsatsord
if mod(ordnummer,2)==0 %ord på jämn "plats" i listan
    motsatsord=ord(ordnummer-1); %motsatsordet är ordet på platsen innan
else
    motsatsord=ord(ordnummer+1); %motsatsordet är ordet på platsen efter
end
